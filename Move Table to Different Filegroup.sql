--https://gallery.technet.microsoft.com/scriptcenter/SQL-Server-Move-Tables-a77e2f1a
...
EXEC spMoveTables
     ...
     2,		 --@TargetFileGroupID
     MyTable,	 --@TableToMove
     1,		 --@MovePKAndAllUniqueConstraints
     1;		 --@MoveAllNonClusteredIndexes

*/
IF OBJECT_ID('spMoveTables') IS NOT NULL
    DROP PROCEDURE [dbo].[spMoveTables];
GO
CREATE PROCEDURE [dbo].[spMoveTables] @SourceFileGroupID             INT,
...
-- Validate the table name and check that it exists in the system catalog.
     IF @TableToMove IS NULL
        OR @TableToMove = ''
         BEGIN
             SET @ScriptMsg = N'The table name provided in the script is either null or empty, on server '+@ServerName+N' and database '+@DatabaseName+N'. Please provide a valid table name.';
             RAISERROR(@ScriptMsg, 16, 1);
             RETURN;
         END;
     IF NOT EXISTS
(
    ...
    WHERE TABLE_NAME = @TableToMove
         ...
         BEGIN
             SET @ScriptMsg = N'The table name provided in the script is not found in database '+@DatabaseName+N', on server '+@ServerName+N'. Please provide a valid table name.';
             RAISERROR(@ScriptMsg, 16, 1);
             RETURN;
         END;
...
(
    ...
    WHERE id = OBJECT_ID(@TableToMove)
         ...
         BEGIN
             SET @ScriptMsg = N'The table '+@TableToMove+' is not found on filegroup '+CAST(@SourceFileGroupID AS VARCHAR(32))+N'. Please provide a valid table name and source file group.';
             RAISERROR(@ScriptMsg, 16, 1);
             RETURN;
         END;
 ...
 IdxOrder
                             ...
                             SELECT COL_NAME(OBJECT_ID(@TableToMove), colid),
                                -- append the DESC/ASC string, based on the ASC/DESC order of the PK columns
                                    CASE
                                        WHEN INDEXKEY_PROPERTY(OBJECT_ID(@TableToMove), INDEXPROPERTY(OBJECT_ID(@TableToMove), @KeyName, 'IndexID'), keyno, 'IsDescending') = 1
                                        THEN 'DESC'
                                        ELSE 'ASC'
                                    END
                             FROM sysindexkeys
...
(ColName,
 IdxOrder
             ...
             SELECT COL_NAME(OBJECT_ID(@TableToMove), colid),
                -- append the DESC/ASC string, based on the ASC/DESC order of the PK columns
                    CASE
                        WHEN INDEXKEY_PROPERTY(OBJECT_ID(@TableToMove), INDEXPROPERTY(OBJECT_ID(@TableToMove), @CIName, 'IndexID'), keyno, 'IsDescending') = 1
                        THEN 'DESC'
                        ELSE 'ASC'
                    END
 ...
 KeyName
                     ...
                     WHILE @AssocFKeyName IS NOT NULL
                        ...
                        -- drop/recreate operation.
                             INSERT INTO #tblFKColTable
(ColName,
 FOrP
)
                             SELECT COL_NAME(fkeyid, fkey),
                                    'F'
                             FROM sysforeignkeys
...
(ColName,
 FOrP
                        ...
                        -- We now build the FK creation statement
                             SELECT @j = MIN(Idx)
                             FROM #tblFKColTable;
                             SET @SQLStr = N'ALTER TABLE ['+@FKTableName+N'] '+N' WITH NOCHECK ADD CONSTRAINT ['+@AssocFKeyName+N'] '+N' FOREIGN KEY (';
...
(KeyName,
 ...
 type
                    ...
                    FROM sysindexes a WITH (NOLOCK)
                         INNER JOIN sysobjects b WITH (NOLOCK) ON a.[name] = b.[name]
...
(KeyName,
 ...
 type
                    ...
                    SELECT a.[name],
                           ...
                           END
                    FROM sysindexes a WITH (NOLOCK)
                         LEFT OUTER JOIN #tblKeysTable b ON a.name = b.KeyName
