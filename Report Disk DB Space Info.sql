...
select db_name(database_id) DBName, convert(decimal(15,2),sum(size)/128.0) DBSizeMB into #HelpDB from sys.master_files
...
,fileproperty(name,''''IsPrimaryFile'''') IsPrimaryFile, fileproperty(name,''''IsLogFile'''') from sysfiles''');
        ...
        FROM sys.master_files AS f
			 ...
			 inner join #HelpDB HD on db_name(f.database_id) = HD.DBName
